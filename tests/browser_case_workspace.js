async (page) => {
  ...
  for (const locator of [
    page.getByRole('button', { name: '설명자료 만들기', exact: true }),
    page.getByRole('button', { name: '계좌가 막힌 후 준비', exact: true }).first(),
    page.getByRole('button', { name: '계좌가 막힌 후 준비', exact: true }).last(),
  ]) {
    await locator.click();
    check(page.url().endsWith('#s00'), 'Home must open Frozen start, not dashboard');
    await page.goto('http://127.0.0.1:8876/#home');
  }
  await page.getByRole('button', { name: '설명자료 만들기', exact: true }).click();
  await page.getByRole('button', { name: '예시 사건으로 체험', exact: true }).click();
  await page.getByRole('button', { name: '메시지 붙여넣기', exact: true }).click();
  await page.locator('#message').fill('금융감독원입니다. 지금 안전계좌로 이체하세요. 누구에게도 말하지 마세요.');
  await page.getByRole('button', { name: '이 메시지 점검하기', exact: true }).click();
  ...
  await page.getByRole('button', { name: '정리할 자료 확인', exact: true }).click();
  ...
  await page.getByRole('button', { name: '다음 단계: 거래 연결', exact: true }).click();
  ...
  await page.getByRole('button', { name: '다음 단계: 확인할 문제', exact: true }).click();
  await page.getByRole('button', { name: '다음 단계: 시간순 기록', exact: true }).click();
  ...
  await page.getByRole('button', { name: '다음 단계: 결과 보고서', exact: true }).click();
  ...
  await page.getByRole('button', { name: '다음 단계: 빠진 자료 보완', exact: true }).click();
